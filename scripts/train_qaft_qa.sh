...
# QAFT-QA corresponds to the legacy simple_sft stage.
# Usually set KNOWLEDGE_MODEL and PROJECTOR_PATH from the previous QAFT-DC checkpoint.
...
: "${TRAIN_FILE:?Set TRAIN_FILE to the QAFT-QA training file.}"
...
args=(
  ...
  --save-steps "${SAVE_STEPS:-150}"
  --log-steps "${LOG_STEPS:-10}"
  --lora-alpha "${LORA_ALPHA:-32}"
  --lora-dropout "${LORA_DROPOUT:-0.05}"
  --lora-r "${LORA_R:-16}"
  --train-file "${TRAIN_FILE}"
  --token-range "${TOKEN_RANGE:-2048~4096}"
  ...
  --kl-weight-initial "${KL_WEIGHT_INITIAL:-0.0}"
  --kl-weight-decay-strategy "${KL_WEIGHT_DECAY_STRATEGY:-no_decay}"
)

if [[ -n "${VAL_FILE:-}" ]]; then args+=(--val-file "${VAL_FILE}"); fi
if [[ -n "${CHECKPOINT_DIR:-}" ]]; then args+=(--checkpoint-dir "${CHECKPOINT_DIR}"); fi
if [[ "${ENABLE_KL_WEIGHT_DECAY:-0}" == "1" ]]; then args+=(--enable-kl-weight-decay); fi
